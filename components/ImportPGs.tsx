...
import { Upload, Share2, CheckCircle2, ChevronLeft, ChevronRight, Plus, RefreshCw, XCircle, ShieldAlert } from 'lucide-react';
...
const ImportPGs: React.FC<ImportPGsProps> = ({ pgs, setPgs, defaultUnit }) => {
  ...
  const handleAnalyze = async (dataToProcess: string) => {
    ...
    try {
        ...
        for (const row of rows) {
            ...
            let status: PGPreview['status'] = 'new';
            if (unit !== defaultUnit) {
                status = 'wrong_unit';
            } else {
                let prefix = unit === 'Belém' ? 'HAB' : 'HABA';
                const finalId = `${prefix}_${idRaw}`;
                presentPgIds.add(finalId);

                const existing = pgs.find(p => p.id === finalId);
                status = existing ? 'unchanged' : 'new';
                if (existing && (!existing.active || existing.name !== pgName || existing.hospital !== unit)) {
                    status = 'updated';
                }
                
                reportList.push({
                    id: finalId, name: pgName, unit, status,
                    rawObj: { id: finalId, name: pgName, hospital: unit, active: true }
                });
            }
        }

        // INATIVAÇÃO ESTRITA: Apenas da unidade alvo
  ...
  const handleConfirmSave = async () => {
      ...
      try {
          ...
          alert("PGs sincronizados com isolamento estrito!");
          ...
          <div className="space-y-6">
            <div className="flex items-center gap-3 text-indigo-700">
               ...
               <div>
                   ...
                   <p className="text-[10px] font-bold text-indigo-600 uppercase tracking-widest">Unidade Alvo: {defaultUnit}</p>
               </div>
            </div>
            <p className="text-xs text-slate-500 font-medium bg-white/70 p-5 rounded-[2rem] border border-indigo-100">
                Lógica Estrita: O sistema gera chaves únicas baseadas na unidade (Ex: HAB_01). PGs de outras unidades serão preservados.
            </p>
            <button onClick={() => fileInputRef.current?.click()} className="w-full p-8 border-4 border-dashed border-indigo-200 rounded-[2.5rem] bg-white hover:bg-indigo-50 hover:border-indigo-400 transition-all group flex flex-col items-center gap-3">
                ...
                <span className="font-black text-[11px] uppercase text-indigo-600 tracking-widest">Subir Planilha Estrita</span>
            </button>
          </div>
          <div className="lg:col-span-2 flex flex-col gap-4">
              <textarea className="w-full flex-1 h-40 p-8 rounded-[2rem] border-2 border-indigo-100 outline-none font-mono text-xs resize-none bg-white shadow-inner" placeholder="ID; Nome PG; Unidade..." value={pasteData} onChange={e => setPasteData(e.target.value)} />
              <button onClick={() => handleAnalyze(pasteData)} disabled={isAnalyzing || !pasteData.trim()} className="w-full py-5 bg-indigo-600 text-white rounded-2xl font-black uppercase text-xs tracking-widest shadow-xl shadow-indigo-200 hover:bg-indigo-700">
                {isAnalyzing ? 'Processando Unidade...' : 'Analisar PGs'}
              </button>
          </div>
      </div>
      ...
      {showPreview && (
          <div className="animate-in slide-in-from-bottom-10 fade-in duration-500">
                  ...
                  <table className="w-full text-left">
                      <thead className="bg-slate-50 border-b border-slate-200 text-[10px] uppercase font-black text-slate-400 tracking-widest">
                          <tr>
                              ...
                              <th className="p-6">ID Gerado</th>
                              <th className="p-6">Nome PG</th>
                              ...
                              <tr key={item.id} className={`border-b border-slate-50 last:border-0 ${item.status === 'inactivated' ? 'bg-red-50/50' : item.status === 'wrong_unit' ? 'bg-orange-50/30' : 'hover:bg-slate-50'}`}>
                                  <td className="p-6">
                                      {item.status === 'wrong_unit' ? <span className="text-orange-600 font-black">IGNORADO</span> : 
                                       ...
                                       item.status === 'inactivated' ? <span className="text-red-600">INATIVAR</span> : 'OK'}
                                  </td>
              ...
              <div className="flex justify-end gap-4">
                  ...
                  <button onClick={handleConfirmSave} disabled={isSaving} className="px-10 py-4 bg-indigo-600 text-white rounded-2xl font-black uppercase text-xs tracking-widest shadow-xl hover:bg-indigo-700 flex items-center gap-2">
