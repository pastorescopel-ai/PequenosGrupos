         ...
         <form onSubmit={handleSubmit} className="space-y-8">
            ...
            <div className="relative">
               ...
               {searchTerm.length > 1 && !selectedCollabFromRH && (
                 <div className="absolute top-full left-0 right-0 mt-2 bg-white border border-slate-200 rounded-2xl shadow-2xl overflow-hidden z-[140] max-h-80 overflow-y-auto custom-scrollbar">
                        ...
                        searchResults.map(res => (
                          <button key={res.employee_id} type="button" onClick={() => selectCollab(res)} className="w-full p-5 text-left hover:bg-blue-50 flex items-center gap-4 border-b border-slate-100 last:border-0 group">
                             ...
                             <div className="flex-1">
                                ...
                                <p className="text-[10px] font-black text-blue-600 uppercase tracking-widest">SETOR: {res.sector_name}</p>
                                <p className="text-[9px] text-slate-400 font-bold uppercase">Matrícula: {res.employee_id}</p>
                             </div>
