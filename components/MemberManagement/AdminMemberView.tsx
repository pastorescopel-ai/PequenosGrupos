...
import { Search, Users, ChevronLeft, AlertCircle, Clock, MapPin } from 'lucide-react';
import { Leader, ChangeRequest, Collaborator, PG, Sector, HospitalUnit } from '../../types';
...
const AdminMemberView: React.FC<AdminMemberViewProps> = ({ 
  ...
  useEffect(() => {
      if (initialPGId) {
          ...
          if (pg) {
              setSelectedPG(pg);
              // Sincroniza a unidade do PG selecionado se houver informação
              if (pg.hospital) setSelectedUnit(pg.hospital);
  ...
  const pgCardsData = useMemo(() => {
    ...
    return pgsWithStats
      .filter(p => 
        p.active && 
        (p.hospital === selectedUnit || (!p.hospital && selectedUnit === 'Belém')) && // SHIELD-V43: Filtro de Unidade
        (!pgSearch || p.name.toLowerCase().includes(pgSearch.toLowerCase()) || p.leaderName.toLowerCase().includes(pgSearch.toLowerCase()))
      )
      .sort((a, b) => {
  ...
  }, [pgs, leaders, members, memberRequests, pgSearch, selectedUnit]);
    ...
    <div className="space-y-10 animate-in fade-in duration-500 text-left">
        <header className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
            <div>
                ...
                <p className="text-slate-500 font-medium">Filtre por unidade e selecione um grupo para gerenciar membros.</p>
            </div>
            
            <div className="flex flex-col sm:flex-row items-center gap-4">
                {/* SELETOR DE UNIDADE PILL STYLE V43 */}
                <div className="bg-white p-1.5 rounded-2xl shadow-sm border border-slate-100 flex items-center gap-2 h-[60px]">
                     <div className="flex bg-slate-100 p-1 rounded-xl h-full items-center">
                        <button 
                            onClick={() => setSelectedUnit('Belém')} 
                            className={`flex items-center gap-2 px-6 py-2.5 rounded-lg text-[10px] font-black uppercase transition-all ${selectedUnit === 'Belém' ? 'bg-white text-blue-600 shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}
                        >
                            <MapPin size={14}/> HAB
                        </button>
                        <button 
                            onClick={() => setSelectedUnit('Barcarena')} 
                            className={`flex items-center gap-2 px-6 py-2.5 rounded-lg text-[10px] font-black uppercase transition-all ${selectedUnit === 'Barcarena' ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}
                        >
                            <MapPin size={14}/> HABA
                        </button>
                     </div>
                ...
                <div className="relative w-full md:w-80 group">
                    ...
                    <input 
                        type="text" 
                        placeholder="Buscar PG ou Líder..." 
                        value={pgSearch} 
                        onChange={(e) => setPgSearch(e.target.value)} 
                        className="w-full pl-12 pr-4 py-4 bg-white border border-slate-200 rounded-3xl font-bold text-slate-800 outline-none focus:border-blue-600 shadow-sm h-[60px]" 
                    />
                </div>
            </div>
        </header>

        {pgCardsData.length === 0 ? (
            <div className="py-24 text-center bg-white rounded-[3rem] border-2 border-dashed border-slate-100">
                <div className="w-20 h-20 bg-slate-50 rounded-3xl flex items-center justify-center mx-auto mb-6 text-slate-200">
                    <Users size={40} />
                </div>
                <h3 className="text-xl font-black text-slate-300 uppercase tracking-widest">Nenhum PG encontrado em {selectedUnit}</h3>
                <p className="text-slate-400 text-xs font-medium mt-2">Tente ajustar sua busca ou trocar de unidade.</p>
            </div>
        ) : (
