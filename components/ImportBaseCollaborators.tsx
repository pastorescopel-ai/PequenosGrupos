
import React, { useState, useRef, useMemo } from 'react';
import { doc, writeBatch } from 'firebase/firestore';
import { db } from '../lib/firebase';
import { Collaborator, HospitalUnit, Sector } from '../types';
import { Upload, FileText, CheckCircle2, RefreshCw, ChevronLeft, ChevronRight, Database, UserPlus, Trash2, Edit3, PlusCircle } from 'lucide-react';

interface ImportBaseCollaboratorsProps {
  allCollaborators: Collaborator[]; 
  setAllCollaborators: React.Dispatch<React.SetStateAction<Collaborator[]>>;
  sectors: Sector[]; 
  defaultUnit: HospitalUnit;
}

interface PreviewItem {
  id: string;
  name: string;
  sector: string;
  unit: string;
  currentPg?: string;
  status: 'new' | 'updated' | 'inactivated' | 'unchanged' | 'corrected' | 'wrong_unit';
  details?: string;
  rawObj?: Collaborator;
}

const ImportBaseCollaborators: React.FC<ImportBaseCollaboratorsProps> = ({ allCollaborators, setAllCollaborators, sectors = [], defaultUnit }) => {
  const [pasteData, setPasteData] = useState('');
  const [isAnalyzing, setIsAnalyzing] = useState(false);
  const [isSaving, setIsSaving] = useState(false);
  const [previewData, setPreviewData] = useState<PreviewItem[]>([]);
  const [currentPage, setCurrentPage] = useState(1);
  const itemsPerPage = 10;
  const [showPreview, setShowPreview] = useState(false);
  const fileInputRef = useRef<HTMLInputElement>(null);

  const stats = useMemo(() => {
    return {
      new: previewData.filter(i => i.status === 'new').length,
      updated: previewData.filter(i => i.status === 'updated' || i.status === 'corrected').length,
      inactivated: previewData.filter(i => i.status === 'inactivated').length,
      total: previewData.filter(i => i.status !== 'wrong_unit').length
    };
  }, [previewData]);

  const handleAnalyze = async (dataToProcess: string) => {
    if (!dataToProcess.trim()) return;
    setIsAnalyzing(true);
    setPreviewData([]);
    
    try {
      const allRows = dataToProcess.split('\n').map(l => l.trim()).filter(l => l !== "");
      
      // V40/41: SALTO DE CABEÇALHO (Linha 1 ignorada)
      const dataRows = allRows.slice(1);
      
      const presentIds = new Set<string>(); 
      const reportList: PreviewItem[] = [];

      for (const row of dataRows) {
        const parts = row.includes(';') ? row.split(';') : row.split('\t'); 
        if (parts.length < 2) continue;

        const matricula = parts[0]?.trim() || '';
        const nome = parts[1]?.trim().toUpperCase() || '';
        const idSetor = parts[2]?.trim() || '';
        let nomeSetor = parts[3]?.trim() || 'GERAL';
        const unidadeRaw = parts[4]?.trim().toLowerCase();

        // V40: FILTRO DE INTEGRIDADE
        if (!matricula || !nome) continue;

        let targetUnit: HospitalUnit = defaultUnit;
        if (unidadeRaw) {
            if (unidadeRaw.includes('barcarena') || unidadeRaw.includes('haba')) targetUnit = 'Barcarena';
            else if (unidadeRaw.includes('belem') || unidadeRaw.includes('hab')) targetUnit = 'Belém';
        }

        if (targetUnit !== defaultUnit) continue;

        const existing = allCollaborators.find(c => c.employee_id === matricula);
        let status: PreviewItem['status'] = existing ? 'unchanged' : 'new';
        let details = '';

        if (idSetor && sectors.length > 0) {
            const officialSector = sectors.find(s => s.code === idSetor);
            if (officialSector && nomeSetor !== officialSector.name) {
                status = 'corrected';
                details = `Setor normalizado.`;
                nomeSetor = officialSector.name;
            }
        }

        if (existing && !existing.active) { status = 'updated'; details = 'Reativação.'; }
        else if (existing && existing.sector_name !== nomeSetor) { status = 'updated'; details = 'Mudança Setor.'; }

        presentIds.add(matricula);
        reportList.push({
            id: matricula, name: nome, sector: nomeSetor, unit: targetUnit, 
            currentPg: existing?.pg_name || '---', status, details,
            rawObj: { id: matricula, employee_id: matricula, full_name: nome, sector_id: idSetor, sector_name: nomeSetor, active: true, hospital: targetUnit, pg_name: existing?.pg_name } as any
        });
      }

      // INATIVAÇÃO ESTRITA
      const toInactivate = allCollaborators.filter(c => 
          c.active && 
          (c.hospital === defaultUnit || (!c.hospital && defaultUnit === 'Belém')) &&
          !presentIds.has(c.employee_id)
      );
      
      toInactivate.forEach(c => {
          reportList.push({ id: c.employee_id, name: c.full_name, sector: c.sector_name, unit: c.hospital || 'Belém', status: 'inactivated', details: 'Removido da planilha.' });
      });

      // V41: ORDENAÇÃO POR PRIORIDADE (Inativados primeiro, depois Alfabético)
      const sortedList = reportList.sort((a, b) => {
          if (a.status === 'inactivated' && b.status !== 'inactivated') return -1;
          if (a.status !== 'inactivated' && b.status === 'inactivated') return 1;
          return a.name.localeCompare(b.name);
      });

      setPreviewData(sortedList);
      setShowPreview(true);
      setCurrentPage(1);
    } catch (e) { alert("Análise falhou."); } 
    finally { setIsAnalyzing(false); }
  };

  const handleConfirmSave = async () => {
      setIsSaving(true);
      try {
          const batchSize = 400; 
          let batch = writeBatch(db!);
          let opCount = 0;

          const toSave = previewData.filter(i => (i.status === 'new' || i.status === 'updated' || i.status === 'corrected') && i.rawObj);
          for (const item of toSave) {
              batch.set(doc(db!, "collaborators", item.id), item.rawObj, { merge: true });
              opCount++;
              if (opCount >= batchSize) { await batch.commit(); batch = writeBatch(db!); opCount = 0; }
          }

          const toInactivate = previewData.filter(i => i.status === 'inactivated');
          for (const item of toInactivate) {
              batch.update(doc(db!, "collaborators", item.id), { active: false });
              opCount++;
              if (opCount >= batchSize) { await batch.commit(); batch = writeBatch(db!); opCount = 0; }
          }

          if (opCount > 0) await batch.commit();
          alert("Sincronização de RH concluída com sucesso!");
          setShowPreview(false);
          setPasteData('');
      } catch (e) { alert("Erro ao gravar no banco."); } 
      finally { setIsSaving(false); }
  };

  const indexOfLastItem = currentPage * itemsPerPage;
  const indexOfFirstItem = indexOfLastItem - itemsPerPage;
  const currentItems = previewData.slice(indexOfFirstItem, indexOfLastItem);
  const totalPages = Math.ceil(previewData.length / itemsPerPage);

  return (
    <div className="space-y-10 text-left">
      <div className="bg-blue-50/50 p-8 rounded-[3rem] border-2 border-blue-100 grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="space-y-6">
          <div className="flex items-center gap-3 text-blue-800"><div className="p-3 bg-white rounded-2xl shadow-sm"><FileText size={24}/></div><div><h4 className="text-xl font-black">Base RH Oficial</h4><p className="text-[10px] font-black uppercase tracking-widest text-blue-600">{defaultUnit}</p></div></div>
          <div className="bg-white/70 p-5 rounded-[2rem] border border-blue-100 space-y-2">
                <p className="text-[10px] font-black text-blue-800 uppercase tracking-widest">Integridade V41:</p>
                <ul className="text-[11px] text-slate-500 font-medium space-y-1">
                    <li>• Inativações aparecem no topo do preview.</li>
                    <li>• Paginação ativa para auditoria rápida.</li>
                    <li>• Filtro automático de linhas vazias.</li>
                </ul>
          </div>
          <button onClick={() => fileInputRef.current?.click()} className="w-full p-6 border-4 border-dashed border-blue-200 rounded-[2.5rem] bg-white hover:bg-blue-50 transition-all flex flex-col items-center gap-2">
            <input type="file" ref={fileInputRef} className="hidden" accept=".csv,.txt" onChange={(e) => { const f = e.target.files?.[0]; if(f){ const r = new FileReader(); r.onload=(ev)=>handleAnalyze(ev.target?.result as string); r.readAsText(f); } }}/>
            <Upload size={28} className="text-blue-400" /><span className="font-black text-[10px] uppercase text-blue-600">Subir Planilha</span>
          </button>
        </div>
        <div className="lg:col-span-2 flex flex-col gap-4">
           <textarea className="w-full flex-1 h-[250px] p-6 rounded-[2.5rem] border-2 border-blue-100 outline-none font-mono text-xs resize-none bg-white shadow-inner" placeholder="Cole aqui seus dados do Excel (Iniciando pela linha de títulos)..." value={pasteData} onChange={e => setPasteData(e.target.value)} />
           <button onClick={() => handleAnalyze(pasteData)} disabled={isAnalyzing || !pasteData.trim()} className="w-full py-5 bg-blue-600 text-white rounded-2xl font-black uppercase text-xs tracking-widest shadow-xl flex items-center justify-center gap-3">
             {isAnalyzing ? <><RefreshCw size={18} className="animate-spin"/> Analisando Base...</> : 'Analisar e Contar'}
           </button>
        </div>
      </div>

      {showPreview && (
          <div className="animate-in slide-in-from-bottom-10 duration-500 pb-12">
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                  <StatCard label="Novos" value={stats.new} icon={<PlusCircle size={16}/>} color="bg-green-50 text-green-700 border-green-100" />
                  <StatCard label="Alterar" value={stats.updated} icon={<Edit3 size={16}/>} color="bg-blue-50 text-blue-700 border-blue-100" />
                  <StatCard label="Inativar" value={stats.inactivated} icon={<Trash2 size={16}/>} color="bg-red-50 text-red-700 border-red-100" />
                  <StatCard label="Total RH" value={stats.total} icon={<Database size={16}/>} color="bg-slate-50 text-slate-700 border-slate-200" />
              </div>

              <div className="bg-white border border-slate-200 rounded-[2.5rem] overflow-hidden shadow-sm mb-8 overflow-x-auto">
                  <table className="w-full text-left">
                    <thead className="bg-slate-50 border-b border-slate-200 text-[10px] uppercase font-black text-slate-400 tracking-widest">
                        <tr><th className="p-6">Status</th><th className="p-6">Matrícula</th><th className="p-6">Colaborador</th><th className="p-6">Setor Planilha</th></tr>
                    </thead>
                    <tbody className="text-xs font-medium text-slate-600">
                        {currentItems.map((item) => (
                            <tr key={item.id} className={`border-b border-slate-50 last:border-0 ${item.status === 'inactivated' ? 'bg-red-50/30' : ''}`}>
                                <td className="p-6">
                                    {item.status === 'new' ? <span className="text-green-600 font-black">NOVO</span> :
                                     item.status === 'inactivated' ? <span className="text-red-600 font-black">INATIVAR</span> :
                                     item.status === 'updated' || item.status === 'corrected' ? <span className="text-blue-600 font-black">ATUALIZAR</span> :
                                     <span className="text-slate-400">MANTIDO</span>}
                                </td>
                                <td className="p-6 font-mono font-bold text-blue-600">{item.id}</td>
                                <td className="p-6 font-bold">{item.name}</td>
                                <td className="p-6">{item.sector}</td>
                            </tr>
                        ))}
                    </tbody>
                  </table>
                  {totalPages > 1 && (
                      <div className="bg-slate-50 p-6 flex items-center justify-between border-t border-slate-100">
                          <button onClick={() => setCurrentPage(p => Math.max(1, p-1))} className="p-2 hover:bg-white rounded-xl transition-all shadow-sm"><ChevronLeft size={20}/></button>
                          <span className="text-[10px] font-black uppercase text-slate-400 tracking-widest">Página {currentPage} de {totalPages}</span>
                          <button onClick={() => setCurrentPage(p => Math.min(totalPages, p+1))} className="p-2 hover:bg-white rounded-xl transition-all shadow-sm"><ChevronRight size={20}/></button>
                      </div>
                  )}
              </div>

              <div className="flex justify-end gap-4">
                  <button onClick={() => setShowPreview(false)} className="px-8 py-4 text-slate-500 font-bold hover:bg-slate-100 rounded-2xl">Descartar</button>
                  <button onClick={handleConfirmSave} disabled={isSaving} className="px-10 py-4 bg-blue-600 text-white rounded-2xl font-black uppercase text-xs tracking-widest shadow-xl flex items-center gap-3">
                      {isSaving ? <><RefreshCw size={18} className="animate-spin"/> Sincronizando...</> : `Confirmar Sincronização de ${stats.total} Itens`}
                  </button>
              </div>
          </div>
      )}
    </div>
  );
};

const StatCard = ({ label, value, icon, color }: any) => (
    <div className={`p-5 rounded-2xl border flex flex-col gap-1 ${color} shadow-sm`}>
        <div className="flex items-center gap-2 opacity-70">
            {icon}
            <span className="text-[9px] font-black uppercase tracking-widest">{label}</span>
        </div>
        <span className="text-2xl font-black">{value}</span>
    </div>
);

export default ImportBaseCollaborators;
